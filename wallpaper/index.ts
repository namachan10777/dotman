...
function renderCircle(ctx: CanvasRenderingContext2D, r1: number, r2: number, theta_step: number, theta_diff: number, theta_offset: number, color: string) {
...
function render() {
  ...
  if (ctx) {
    ...
    renderCircle(ctx, 110 * scale, 250 * scale, -Math.PI / 180 * 20, Math.PI / 180 * 10, 0.9, 'rgb(200, 200, 200, 0.4)');
  }
